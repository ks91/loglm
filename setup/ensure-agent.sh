...
if (($# != 1)); then
  say "使い方: $0 <codex|claude|gemini|openclaw|hermes|local-llm>" \
      "Usage: $0 <codex|claude|gemini|openclaw|hermes|local-llm>" >&2
  exit 2
fi

AGENT="$1"
case "$AGENT" in
  codex|claude|gemini|openclaw|hermes|local-llm) ;;
  *)
